#!/bin/bash
# Generates Assets.car for macOS builds from the ghostfox branding icons
# This script must be run on macOS (requires actool from Xcode)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BRANDING_DIR="$SCRIPT_DIR/../additions/browser/branding/ghostfox"
...
echo "Generating Assets.car from ghostfox branding icons..."
