...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move Ghostfox/Ghostfox.app -> Ghostfox.app
            nightly_dir = os.path.join(temp_dir, 'Ghostfox')
            shutil.move(
                os.path.join(nightly_dir, 'Ghostfox.app'), os.path.join(temp_dir, 'Ghostfox.app')
            )
            # Remove old app dir and all content in it
            shutil.rmtree(nightly_dir)
        else:
            # Move contents out of camoufox folder if it exists
            old_camoufox_dir = os.path.join(temp_dir, 'ghostfox')
            camoufox_dir = os.path.join(temp_dir, 'ghostfox-folder')
        ...
        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'Ghostfox.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir
...
def main():
    ...
    new_name = f'ghostfox-{args.version}-{args.release}-{args.os[:3]}.{args.arch}.zip'
